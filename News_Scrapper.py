...
class GeneralNews():
    ...
    def store_news_in_database(self, table_name):
        ...
        self.main_news_dataframe["posted_date"] = self.main_news_dataframe["posted_date"].astype(str)
        table = db_name[table_name]
        # deleting old data from trhe table# dropping the existing data
        table.drop()
        # inserting new data
        table.insert_many(self.main_news_dataframe.to_dict("records"))
        print(f"{table_name} has been loaded with data")
